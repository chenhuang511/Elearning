    ...
    private function generateSchemaRules(){
        foreach ($this->_schema->getSchema() as $field_name => $field){
            ...
            foreach ($validators as $validator_name => $validator){
                ...
                //thêm 2 điều kiện sau để check cho trường username của moodle
                ...
                if ($validator_name == "username_check"){
                    $this->ruleWithMessage("regex", $message_set, $field_name, "/^[a-z0-9_@\-\.]+$/");
                }
                //Only lowercase letters allowed
                if ($validator_name == "only_lowercase"){
                    $this->ruleWithMessage("regex", $message_set, $field_name, "/^[^A-Z]+$/");
                }

                //validate dữ liệu của password trong moodle
                //link : http://stackoverflow.com/questions/19605150/regex-for-password-must-be-contain-at-least-8-characters-least-1-number-and-bot
                if ($validator_name == "check_pass"){
                    $this->ruleWithMessage("regex", $message_set, $field_name, "/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[$@$!%*?&])[A-Za-z\d$@$!%*?&]{8,}/");
                }
            }
        }
    }    

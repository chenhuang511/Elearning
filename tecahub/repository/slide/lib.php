...
require_once(dirname(dirname(__FILE__)) . '/url/locallib.php');
require_once(dirname(__FILE__) . '/sliderepo.php');

/**
 * repository_slide class
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class repository_slide extends repository {
...
EOD;
    ...
    public function get_listing($path='', $page='') {
        $client = new sliderepo();
        $list = array();
        $list['page'] = (int)$page;
        if ($list['page'] < 1) {
            $list['page'] = 1;
        }
        $list['list'] = $client->get_slides($page);
        $list['nologin'] = true;
        $list['norefresh'] = true;
        $list['nosearch'] = true;
        return $list;
    ...
    protected function parse_file($baseurl, $relativeurl, &$list, $mainfile = false) {
        ...
        if ($info['http_code'] != 200) {
            ...
            if ($mainfile && (strstr($info['content_type'], 'text/html') || empty($info['content_type']))) {
                ...
                $list['error'] = get_string('validfiletype', 'repository_slide');
            }

 ...
 *
 * @package    core_local_slide
 * @category   external
 * @copyright  2011 Jerome Mouneyrac
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since Moodle 2.2
 */
class local_mod_slide_external extends external_api
{
    /**
     ...
     */
    public static function get_slide_by_id($id)
    {
        global $DB;
        //validate parameter
        $params = self::validate_parameters(self::get_slide_by_id_parameters(),
            array('id' => $id));
        $warnings = array();
        // Entry to return.
        $slide = null;
        if (!empty($params['id'])) {
            $query = "SELECT slide.id, slide.course, slide.name, slide.intro, slide.introformat, 
                        slide.externalurl, slide.display, slide.displayoptions, slide.parameters, slide.timemodified
                        FROM {slide} slide
                        WHERE slide.id = ?";
            $slide = $DB->get_record_sql($query, $params);
        }
        $return = array();
        $return['slide'] = $slide;
    ...
    public static function get_slide_by_id_returns()
    {
        return new external_single_structure(
            array(
                'slide' => new external_single_structure(
                    array(

...
class local_mod_quiz_external extends external_api {
    ...
    /**
     * Hanv 24/05/2016
     * Return all the question of quiz and question option
     *
     * @return external_function_parameters
     * @since Moodle 2.9 Options available
     * @since Moodle 2.2
     *
     */
    public static function get_mod_quiz_question_parameters() {
        return new external_function_parameters(
            array('attemptid' => new external_value(PARAM_INT, 'attempt id'),
                'options' => new external_multiple_structure (
                    new external_single_structure(
                        array(
                            'name' => new external_value(PARAM_ALPHANUM,
     ...
     * @since Moodle 2.2
     */
    public static function get_mod_quiz_question($attemptid, $options = array()) {
        global $CFG, $DB;
        ...
        // Get mnethost by ipadress
        $quizid =  $DB->get_record('mnet_host', array('ip_address' => $params['ip_address']), '*', MUST_EXIST);
     ...
     * @since Moodle 2.2
     */
    public static function get_mod_quiz_question_returns() {
        return  new external_single_structure(
            array(
                'id' => new external_value(PARAM_INT, 'attempt id'),
                'quiz' => new external_value(PARAM_INT, 'quiz id'),

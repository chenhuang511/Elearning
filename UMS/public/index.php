<?php
    ...
    // List cohort
    $app->get('/list/cohorts/?', function () use ($app){
        $controller = new UF\MdlCohortController($app);
        return $controller->listAllCohort();
    });

    // List category cohort
    $app->get('/list/cohorts/ct/:context_id/?', function ($context_id) use ($app){
        $controller = new UF\MdlCohortController($app);
        return $controller->listCatCohort($context_id);
    });

    // Form create cohort
    $app->get('/forms/cohorts/ct/:context_id/?', function ($context_id) use ($app){
        $controller = new UF\MdlCohortController($app);
        return $controller->CreateCohort($context_id);
    });

    // Form edit cohort
    $app->get('/forms/cohorts/c/:cohort_id/:context_id/?', function ($cohort_id, $context_id) use ($app){
        $controller = new UF\MdlCohortController($app);
        return $controller->EditCohort($cohort_id, $context_id);
    });

    // Assign cohort member
    $app->get('/assign/cohorts/c/:cohort_id/:context_id/?', function ($cohort_id, $context_id) use ($app){
        $controller = new UF\MdlCohortController($app);
        return $controller->assignCohortMember($cohort_id, $context_id);
    });

    // Search
    $app->get('/form/search/c/:cohort_id/?', function($cohort_id) use ($app){
        $controller = new UF\MdlCohortController($app);
        return $controller->searchMember($cohort_id);
    });

    // Create cohort
    $app->post('/forms/cohorts/?', function () use ($app){
        $controller = new UF\MdlCohortController($app);
        return  $controller->submitCreateCohort();
    });

    // Edit cohort
    $app->post('/forms/cohorts/c/:cohort_id/?', function ($cohort_id) use ($app){
        $controller = new UF\MdlCohortController($app);
        return $controller->submitEditCohort($cohort_id);
    });

    // Assign submit cohort member
    $app->post('/assign/cohorts/c/:cohort_id/:context_id/?', function ($cohort_id, $context_id) use ($app){
        $controller = new UF\MdlCohortController($app);
        return $controller->assignSubmitCohortMember($cohort_id, $context_id);
    });

    // Delete cohort
    $app->post('/forms/cohorts/c/:cohort_id/delete/?', function ($cohort_id) use ($app) {
        $controller = new UF\MdlCohortController($app);

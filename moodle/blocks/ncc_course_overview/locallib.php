 ...
 *
 * @package    block_ncc_course_overview
 * @copyright  2016 Nccsoft Vietnam <http://nccsoft.vn>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

define('BLOCKS_NCC_COURSE_OVERVIEW_SHOWCATEGORIES_NONE', '0');
define('BLOCKS_NCC_COURSE_OVERVIEW_SHOWCATEGORIES_ONLY_PARENT_NAME', '1');
define('BLOCKS_NCC_COURSE_OVERVIEW_SHOWCATEGORIES_FULL_PATH', '2');
 ...
 */
function block_ncc_course_overview_get_overviews($courses) {
    $htmlarray = array();
 ...
 */
function block_ncc_course_overview_update_mynumber($number) {
    set_user_preference('ncc_course_overview_number_of_courses', $number);
}
 ...
 */
function block_ncc_course_overview_update_myorder($sortorder) {
    ...
    set_user_preference('ncc_course_overview_course_sortorder', $value);
}
 ...
 */
function block_ncc_course_overview_get_myorder() {
    if ($value = get_user_preferences('ncc_course_overview_course_sortorder')) {
        return explode(',', $value);
    }
    // If preference was not found, look in the old location and convert if found.
    $order = array();
    if ($value = get_user_preferences('ncc_course_overview_course_order')) {
        $order = unserialize($value);
        block_course_overview_update_myorder($order);
        unset_user_preference('ncc_course_overview_course_order');
    }
    return $order;
}
 ...
 */
function block_ncc_course_overview_get_child_shortnames($courseid) {
    global $DB;
    ...
    if ($results = $DB->get_records_sql($sql, $params)) {
        ...
        if ($total > 10) {
            ...
            if ($diff > 1) {
                $suffix = get_string('shortnamesufixprural', 'block_ncc_course_overview', $diff);
            } else {
                $suffix = get_string('shortnamesufixsingular', 'block_ncc_course_overview', $diff);
            }
        }
        $shortnames = get_string('shortnameprefix', 'block_ncc_course_overview', implode('; ', $shortnames));
        $shortnames .= $suffix;
    }
 ...
 */
function block_ncc_course_overview_get_max_user_courses($showallcourses = false) {
    ...
    if (empty($config->forcedefaultmaxcourses)) {
        if ($showallcourses) {
            ...
            $limit = get_user_preferences('ncc_course_overview_number_of_courses', $limit);
        }
    }
    return $limit;
}
 ...
 */
function block_ncc_course_overview_get_sorted_courses($showallcourses = false) {
    global $USER;

    $limit = block_ncc_course_overview_get_max_user_courses($showallcourses);
    ...
    $order = block_ncc_course_overview_get_myorder();

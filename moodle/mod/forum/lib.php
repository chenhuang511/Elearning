...
function forum_print_nondiscussion($forum, $cm = null)
{
...
function forum_add_discussion($discussion, $mform = null, $unused = null, $userid = null)
{
    ...
    if (MOODLE_RUN_MODE === MOODLE_MODE_HUB) {
        ...
        $hubcourseid = $discussion->course;

        $localcourse = $DB->get_record('course', array("id" => $discussion->course));
        ...
        $i = 0;
        foreach ($discussion as $key => $val) {
            $discussiondata["discussiondata[$i][name]"] = $key;
            if ($key == "course") {
                $discussiondata["discussiondata[$i][value]"] = $hubcourseid;
            } else {
                $discussiondata["discussiondata[$i][value]"] = $val;
            }
            $i++;
        }

        $result = save_remote_forum_add_discussions($discussiondata, $hubuserid);
        $forum = $result->forum;
        $post = $result->post;
...
function forum_user_has_posted_discussion($forumid, $userid, $groupid = null)
{
    ...
    if (MOODLE_RUN_MODE === MOODLE_MODE_HUB) {
        ...
        if (count($params) > 2) {
            ...
            $prs['parameters[2][value]'] = $groupid;
        }

        $status = get_remote_forum_user_has_posted_discussion($sql, $prs);
        return $status;
    } else {
...
function forum_user_has_posted($forumid, $did, $userid)
{
    ...
    if (empty($did)) {
        ...
        if (MOODLE_RUN_MODE === MOODLE_MODE_HUB) {
            ...
            $params['parameters[0][value]'] = $forumid;
            $params['parameters[1][name]'] = "userid";
            $params['parameters[1][value]'] = $user[0]->id;
            $status = get_remote_forum_user_has_posted($sql, $params);
            return $status;
        } else {
        ...
        if (MOODLE_RUN_MODE === MOODLE_MODE_HUB) {
            ...
            $params['parameters[1][name]'] = "userid";
            $params['parameters[1][value]'] = $user[0]->id;
            $status = check_remote_record_forum_exists("forum_posts", $params);
        } else {
...
function forum_print_discussion($course, $cm, $forum, $discussion, $post, $mode, $canreply = NULL, $canrate = false)
{
    ...
    if (MOODLE_RUN_MODE === MOODLE_MODE_HUB) {
        $hubuserid = $USER->id;
        $huduser = get_remote_mapping_user($USER->id);
        if ($huduser) {
            $hubuserid = $huduser[0]->id;
        }
        $ownpost = (isloggedin() && $hubuserid == $post->userid);
    } else {
...
function forum_tp_stop_tracking($forumid, $userid = false)
{
    ...
    if (MOODLE_RUN_MODE === MOODLE_MODE_HUB) {
        ...
        $prs['parameters[1][value]'] = $forumid;
        $isexists = check_remote_record_forum_exists("forum_track_prefs", $prs);
    } else {
        $isexists = $DB->record_exists('forum_track_prefs', array('userid' => $userid, 'forumid' => $forumid));
    }
...
// Check the given discussion exists
    if (MOODLE_RUN_MODE === MOODLE_MODE_HUB) {
        ...
        $isexists = check_remote_record_forum_exists("forum_discussions", $params);
    } else {
        $isexists = $DB->record_exists('forum_discussions', array('id' => $discussionid));
    }
    if (!$isexists) {
        return false;
    }

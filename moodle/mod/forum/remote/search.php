...
if (!$search || $showform) {
    ...
    $PAGE->navbar->add($strforums, new moodle_url('/mod/forum/remote/index.php', array('id' => $course->id)));
...
$PAGE->navbar->add($strsearch, new moodle_url('/mod/forum/remote/search.php', array('id' => $course->id)));
...
foreach ($posts as $post) {
    ...
    $fullsubject = "<a href=\"/mod/view.php?f=$forum->id\">" . format_string($forum->name, true) . "</a>";
    if ($forum->type != 'single') {
        $fullsubject .= " -> <a href=\"/mod/discuss.php?d=$discussion->id\">" . format_string($discussion->name, true) . "</a>";
        if ($post->parent != 0) {
            $fullsubject .= " -> <a href=\"/mod/discuss.php?d=$post->discussion&amp;parent=$post->id\">" . format_string($post->subject, true) . "</a>";
        }
    }
    ...
    $fulllink = "<a href=\"/mod/discuss.php?d=$post->discussion#p$post->id\">" . get_string("postincontext", "forum") . "</a>";

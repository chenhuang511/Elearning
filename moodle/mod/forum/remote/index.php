...
$url = new moodle_url('/mod/forum/remote/index.php', array('id'=>$id));
if ($subscribe !== null) {
    require_sesskey();
...
if ($id) {
    if (! $course = get_local_course_record($id, true)) {
...
$digestoptions_selector = new single_select(new moodle_url('/mod/forum/remote/maildigest.php',
    array(
        'backtoindex' => 1,
    )),
    'maildigest',
...
if (!is_null($subscribe)) {
    if (isguestuser() or !$can_subscribe) {
        ...
        redirect(
            new moodle_url('/mod/forum/remote/index.php', array('id' => $id)),
    ...
    $returnto = forum_go_back_to(new moodle_url('/mod/forum/remote/index.php', array('id' => $course->id)));
        ...
        if ($usetracking) {
            if ($forum->trackingtype == FORUM_TRACKING_OFF) {
                ...
                if (($forum->trackingtype == FORUM_TRACKING_FORCED) && ($CFG->forum_allowforcedreadtracking)) {
                    ...
                    $aurl = new moodle_url('/mod/forum/remote/settracking.php', array(
                        'id' => $forum->id,
                        'sesskey' => sesskey(),
                    ));
                    if (!isset($untracked[$forum->id])) {
            ...
            if ($usetracking) {
                if ($forum->trackingtype == FORUM_TRACKING_OFF) {
                    ...
                    if (($forum->trackingtype == FORUM_TRACKING_FORCED) && ($CFG->forum_allowforcedreadtracking)) {
                        ...
                        $aurl = new moodle_url('/mod/forum/remote/settracking.php', array('id'=>$forum->id));
                        if (!isset($untracked[$forum->id])) {
                            $trackedlink = $OUTPUT->single_button($aurl, $stryes, 'post', array('title'=>$strnotrackforum));
                        } else {
                            $trackedlink = $OUTPUT->single_button($aurl, $strno, 'post', array('title'=>$strtrackforum));
...
if (!isguestuser() && isloggedin() && $can_subscribe) {
    ...
    echo html_writer::tag('div',
        html_writer::link(new moodle_url('/mod/forum/remote/index.php', array('id'=>$course->id, 'subscribe'=>1, 'sesskey'=>sesskey())),
            get_string('allsubscribe', 'forum')),
        array('class'=>'helplink'));
    echo html_writer::tag('div',
        html_writer::link(new moodle_url('/mod/forum/remote/index.php', array('id'=>$course->id, 'subscribe'=>0, 'sesskey'=>sesskey())),
